...
const AddressField = (props) => {
    return(
        <div className='px-5 flex flex-col w-full h-1/4 mt-10'>
            <label 
                htmlFor = {`${props.id}`} 
                className='h-1/2 w-full text-2xl text-prussian-blue font-bold'>
                    {props.name}:{props.mandatory && <span className='text-red'>*</span>}
            </label>
            <input 
                id = {`${props.id}`} 
                className={`w-full h-1/2  mt-1 px-3 text-xl font-bold shadow-input-shadow rounded-sm ${props.userValidInput && 'rounded-xl border-4 border-green'}`} 
